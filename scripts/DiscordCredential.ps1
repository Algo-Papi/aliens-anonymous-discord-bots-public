if (-not ("DiscordCredential.NativeMethods" -as [type])) {
    Add-Type -TypeDefinition @"
using System;
using System.Runtime.InteropServices;

namespace DiscordCredential {
...
$script:CredentialTarget = "MIB Discord Bot Suite - Agent K Token"

function Set-AACitationDiscordToken {
    ...
    try {
        ...
        $credential = New-Object DiscordCredential.Credential
        $credential.Type = 1
        ...
        if (-not [DiscordCredential.NativeMethods]::CredWrite(
            [ref]$credential,
            0
        )) {
...
function Get-AACitationDiscordToken {
    $credentialPtr = [IntPtr]::Zero
    if (-not [DiscordCredential.NativeMethods]::CredRead(
        ...
        $code = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
        throw "Discord bot token is not stored (Win32 error $code)."
    }

    try {
        $credential = [Runtime.InteropServices.Marshal]::PtrToStructure(
            $credentialPtr,
            [type][DiscordCredential.Credential]
        )
        ...
        if ($credentialPtr -ne [IntPtr]::Zero) {
            [DiscordCredential.NativeMethods]::CredFree($credentialPtr)
        }
    }
}
